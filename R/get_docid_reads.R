#' Get doc ID reads
#'
#' @description Returns an XML-formatted list that summarizes all the successful reads (total reads and non-robot reads) for all the resources of a given PASTA document ID, where a document ID is of the format “scope.identifier” (excludes revision).
#'
#' @param attr.name
...
#'     }
#' @details GET : https://pasta.lternet.edu/audit/reads/{scope}/{identifier}
#' @export
#' @examples 
...
#'
get_docid_reads <- function(attr.name, package.id, environment = "production"){
