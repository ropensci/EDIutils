...
#'
#' @description
#'     Update a data package in the Environmental Data Initiative repository.
#'
#' @usage pkg_update(path, package.id, environment, user.id, user.pass,
#'     affiliation)
#'
#' @param path
...
pkg_update <- function(path, package.id, environment, user.id, user.pass,
                       affiliation){
  ...
  # Place request
  r <- httr::PUT(
    url = paste0(url_env, '.lternet.edu/package/eml/', sid),
