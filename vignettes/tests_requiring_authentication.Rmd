...
The first line of any test requiring authentication should include `skip_if_logged_out()`. This is an internal use only function located in R/utilities.R. Tests including this line will be skipped unless the R environment variable `EDI_API_KEY` is set, or the legacy `EDI_TOKEN` and `AUTH_TOKEN` variables are set. Authenticate by passing `login(key = "your_key")` or by defining the environment variable `EDI_API_KEY`. 

Note that computationally heavy tests are skipped when using an API key unless the environment variable `RUN_ALL_TESTS` is set to `"true"`.
...
By default, the test suite uses `vcr` cassettes for fast, offline testing:
...
To run tests against the live PASTA staging environment instead of recorded VCR cassettes:
...
2. (Optional) Configure test data package if running package lifecycle tests:
   ```r
   config_test_eml()
   ```
...
5. **Request Throttling**: When running real HTTP requests, the test suite automatically introduces a delay between requests (default 1 second) via `httr` response callbacks to avoid PASTA rate limits (HTTP 429). You can customize or disable this delay via the `EDI_TEST_THROTTLE_DELAY` environment variable:
   ```r
