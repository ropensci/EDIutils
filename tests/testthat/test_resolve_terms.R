...
testthat::test_that('Output should be a data frame', {
  expect_equal(class(resolve_terms(recognized_terms, 'lter')), 'data.frame')
})

# Expect no resolution

testthat::test_that('No resolution expected', {
  output <- resolve_terms(unrecognized_terms, 'lter')
...
testthat::test_that('Expect errors', {
  expect_error(resolve_terms(65))
  expect_error(resolve_terms('air temperature', cv = 'retl'))
  expect_error(resolve_terms('air temperature', messages = T, interactive = T))
})
